...
function iconsClickListener(evt) {
  return new Promise((res, err) => {
    if (selfOrParentCheck(evt, "#sound-off")) {
      if (!audioEl) return;
      audioEl.muted = false;
      // Sound off icon's hidden class sometimes fails to get removed on first time it is clicked
      // No idea how to reproduce it, doesn't happen on my device: hopefully this will be a workaround.
      new Promise((res)=>{
...
function asyncAddAudio() {
  ...
  // required for iOS apparently... PC and most other mobile seems fine without it
  audio.load();
  audio.onerror = (err) => console.log("Audio load failed:", err);
  footerEl.appendChild(audio);
  audioEl = audio;
  audio.oncanplaythrough = () => {
    footerEl.querySelectorAll("#music-attribution, #music-icon-attribution").forEach(el => el.classList.remove("hidden"));
    soundOffIcon.classList.remove("hidden");
    soundOnIcon.classList.add("hidden");
  }
